...
import type { CredentialDefinitionRepository } from './repositories/credential-definition.repository'
@Injectable()
export class CredentialDefinitionService extends BaseService {
  ...
  async createCredentialDefinition(payload: CreateCredDefPayload): Promise<credential_definition> {
    try {
      ...
      } else if (response.credentialDefinition.state === 'finished') {
      ...
      credDefResponse.lastChangedBy = undefined
      credDefResponse.lastChangedDateTime = undefined
  ...
  async getAllCredDefs(payload: GetAllCredDefsPayload): Promise<ICredDefDetails> {
    try {
      ...
      const credDefResponse = {
        ...
        hasPreviousPage: credDefSearchCriteria.pageNumber > 1,
