...
import type { SchemaRepository } from './repositories/schema.repository'
import type { CreateSchemaAgentRedirection, GetSchemaAgentRedirection, ISchemaId } from './schema.interface'
...
export class SchemaService extends BaseService {
    ...
    try {
      ...
      if (type === SchemaTypeEnum.INDY) {
        ...
        if (schema !== null || schema !== undefined) {
          ...
          if (
            Number.isNaN(Number.parseFloat(schema.schemaVersion)) ||
          ...
          if (schema.attributes.length > schemaAttributeLength) {
            ...
            if (duplicateAttributeDisplayNames.length > 0) {
            ...
            let schemaResponseFromAgentService: { response: any }
            ...
            if (responseObj.schema.state === 'finished') {
              ...
              const saveResponse = await this.schemaRepository.saveSchema(schemaDetails)

              const attributesArray = JSON.parse(saveResponse.attributes)
              saveResponse.attributes = attributesArray
              saveResponse.lastChangedBy = undefined
              saveResponse.lastChangedDateTime = undefined
              return saveResponse
            }
            if (responseObj.state === 'finished') {
  ...
  private async storeW3CSchemas(
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    schemaDetails: { schemaId: any; did: string; schemaUrl: any },
    user: string,
    orgId: string,
  ...
  async _createW3CledgerAgnostic(payload: object) {
  ...
  async getSchemaById(schemaId: string, orgId: string): Promise<schema> {
    try {
      ...
      // biome-ignore lint/suspicious/noExplicitAny: <explanation>
      let schemaResponse: { response: any }
      ...
      if (getSchemaDetails?.type === SchemaType.W3C_Schema) {
  ...
  async getSchemas(schemaSearchCriteria: ISchemaSearchCriteria, orgId: string): Promise<ISchemasWithPagination> {
    try {
      ...
      const schemasDetails = response?.schemasResult.map((schemaAttributeItem) => {
        ...
        if (schemaAttributeItem?.organisation) {
          schemaAttributeItem.organisation = undefined
        }
      ...
      const schemasResponse: ISchemasWithPagination = {
        ...
        hasPreviousPage: schemaSearchCriteria.pageNumber > 1,
        nextPage,
