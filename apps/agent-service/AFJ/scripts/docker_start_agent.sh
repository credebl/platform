...
AGENT_API_KEY=${18}
ADMIN_PORT_FILE="$PWD/agent-service/AFJ/port-file/last-admin-port.txt"
INBOUND_PORT_FILE="$PWD/agent-service/AFJ/port-file/last-inbound-port.txt"
ADMIN_PORT=8001
INBOUND_PORT=9001
...
if [ -d "${PWD}/agent-service/AFJ/endpoints" ]; then
...
if [ -d "${PWD}/agent-service/AFJ/agent-config" ]; then
  echo "Endpoints directory exists."
else
  echo "Error: Endpoints directory does not exists."
  mkdir -p ${PWD}/agent-service/AFJ/agent-config
fi

AGENT_ENDPOINT="${PROTOCOL}://${EXTERNAL_IP}:${INBOUND_PORT}"

echo "-----$AGENT_ENDPOINT----"
CONFIG_FILE="${PWD}/agent-service/AFJ/agent-config/${AGENCY}_${CONTAINER_NAME}.json"
...
DOCKER_COMPOSE="${PWD}/agent-service/AFJ/${FILE_NAME}"
