...
ADMIN_PORT_FILE="$PWD/apps/agent-service/AFJ/port-file/last-admin-port.txt"
INBOUND_PORT_FILE="$PWD/apps/agent-service/AFJ/port-file/last-inbound-port.txt"
ADMIN_PORT=8001
INBOUND_PORT=9001
...
if [ -d "${PWD}/apps/agent-service/AFJ/endpoints" ]; then
  echo "Endpoints directory exists."
else
  echo "Error: Endpoints directory does not exists."
  mkdir ${PWD}/apps/agent-service/AFJ/endpoints
fi

if [ -d "${PWD}/apps/agent-service/AFJ/agent-config" ]; then
  echo "Endpoints directory exists."
else
  echo "Error: Endpoints directory does not exists."
  mkdir ${PWD}/apps/agent-service/AFJ/agent-config
fi
...
echo "-----$AGENT_ENDPOINT----"
CONFIG_FILE="${PWD}/apps/agent-service/AFJ/agent-config/${AGENCY}_${CONTAINER_NAME}.json"
...
DOCKER_COMPOSE="${PWD}/apps/agent-service/AFJ/${FILE_NAME}"
...
if [ $? -eq 0 ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    mkdir -p "$PWD/agent-service/AFJ/endpoints"
    cat <<EOF >${ENDPOINT}
    {
