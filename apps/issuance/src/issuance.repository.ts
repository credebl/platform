...
import type { IDeletedIssuanceRecords } from '@credebl/common/interfaces/issuance.interface'
...
import type { IIssuedCredentialSearchParams } from 'apps/api-gateway/src/issuance/interfaces'
...
export class IssuanceRepository {
  ...
  async saveIssuedCredentialDetails(payload: IssueCredentialWebhookPayload): Promise<credentials> {
    try {
      ...
      if (issueCredentialDto?.contextCorrelationId !== 'default') {
      ...
      if (
        ...
        (issueCredentialDto?.state && issueCredentialDto?.credentialData?.proposal?.jsonld?.credential?.['@context'][1])
      ) {
        schemaId =
          issueCredentialDto?.metadata?.['_anoncreds/credential']?.schemaId ||
          issueCredentialDto?.credentialData?.offer?.jsonld?.credential?.['@context'][1] ||
  ...
  async getAllFileDetails(
    ...
    try {
      const fileList = await this.prisma.file_upload.findMany({
        ...
        orderBy: {
          createDateTime: getAllfileDetails.sortBy === 'desc' ? 'desc' : 'asc',
        },
      })
