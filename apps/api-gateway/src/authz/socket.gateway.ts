...
export class SocketGateway implements OnGatewayConnection {
  ...
  async handlAgentSpinUpProccessStartedResponse(client: string, payload: ISocketInterface): Promise<void> {
    this.server.to(payload.clientId).emit('agent-spinup-process-initiated');
  }

  @SubscribeMessage('agent-spinup-process-completed')
  async handlAgentSpinUpProccessSucessResponse(client: string, payload: ISocketInterface): Promise<void> {
    this.server.to(payload.clientId).emit('agent-spinup-process-completed');
  }

  @SubscribeMessage('did-publish-process-initiated')
  async handlDidPublicProcessStarted(client: string, payload: ISocketInterface): Promise<void> {
    this.server.to(payload.clientId).emit('did-publish-process-initiated');
  }

  @SubscribeMessage('did-publish-process-completed')
  async handlDidPublicProcessSuccess(client: string, payload: ISocketInterface): Promise<void> {
    this.server.to(payload.clientId).emit('did-publish-process-completed');
  }

  @SubscribeMessage('invitation-url-creation-started')
  async handleInvitationUrlCreationStartResponse(client: string, payload: ISocketInterface): Promise<void> {
  ...
  async handleInvitationUrlCreationSuccessResponse(client: string, payload: ISocketInterface): Promise<void> {
