...
import type { InvitationMessage } from '@credebl/common/interfaces/agent-service.interface'
...
import {
  ...
  type IAgentConfigure,
  type IAgentProofRequest,
  type IAgentSpinUpSatus,
  type IAgentSpinupDto,
  type IAgentStatus,
  type IBasicMessage,
  type IConnectionDetails,
  type ICreateConnectionInvitation,
  type ICreateOrgAgent,
  type ICreateTenant,
  type IDidCreate,
  type IGetCredDefAgentRedirection,
  type IIssuanceCreateOffer,
  IOrgAgent,
  type IOrgAgentsResponse,
  type IOutOfBandCredentialOffer,
  type IQuestionPayload,
  type IReceiveInvitation,
  type IReceiveInvitationUrl,
  type ISendProofRequestPayload,
  type IStoreAgent,
  type IStoreOrgAgentDetails,
  type ITenantCredDef,
  type ITenantDto,
  type ITenantRecord,
  type ITenantSchema,
  type IUserRequestInterface,
  type IWallet,
  type IWalletProvision,
  type LedgerListResponse,
  type OrgDid,
} from './interface/agent-service.interface'
import type { AgentServiceRepository } from './repositories/agent-service.repository'
...
export class AgentServiceService {
  ...
  validatePlatformConfig(platformConfig: platform_config): void {
    if (!platformConfig) {
      this.logger.error(ResponseMessages.agent.error.platformConfiguration)
    ...
    if (!platformConfig.apiEndpoint) {
      this.logger.error(ResponseMessages.agent.error.apiEndpoint)
    ...
    if (!platformConfig.externalIp) {
      this.logger.error(ResponseMessages.agent.error.externalIp)
    ...
    if (typeof platformConfig.externalIp !== 'string') {
      this.logger.error(ResponseMessages.agent.error.stringExternalIp)
      throw new BadRequestException(ResponseMessages.agent.error.stringExternalIp, {
        cause: new Error(),
        description: ResponseMessages.errorMessages.badRequest,
      })
    }
  ...
  validateAgentProcess(agentProcess: ICreateOrgAgent): void {
    try {
      if (!agentProcess) {
        this.logger.error(ResponseMessages.agent.error.agentProcess)
        throw new BadRequestException(ResponseMessages.agent.error.agentProcess, {
          cause: new Error(),
          description: ResponseMessages.errorMessages.badRequest,
        })
      }
  ...
  private async _getAgentDid(payload: IStoreOrgAgentDetails): Promise<object> {
    ...
    const _ledgerDetails = await this.agentServiceRepository.getGenesisUrl(ledgerId)
  ...
  private constructUrl(agentDetails, getOrgAgentType): string {
    ...
    if (getOrgAgentType.agent === OrgAgentType.SHARED) {
  ...
  private async setPrimaryDidAndLedger(orgId, storeDidDetails, network, _method): Promise<void> {
   ...
   */
  async createSecp256k1KeyPair(_orgId: string): Promise<object> {
    try {
      const platformAdminSpinnedUp = await this.agentServiceRepository.platformAdminAgent(
        CommonConstants.PLATFORM_ADMIN_ORG
      )
  ...
  private async getPlatformAdminAndNotify(clientSocketId: string | undefined): Promise<IOrgAgentsResponse> {
    ...
    if (!platformAdminSpinnedUp) {
      this.logger.error(ResponseMessages.agent.error.notAbleToSpinp)
  ...
  private async createTenantAndNotify(payload: ITenantDto, platformAdminSpinnedUp: IOrgAgentsResponse): Promise<any> {
    ...
    WalletSetupPayload.label = undefined
    WalletSetupPayload.clientSocketId = undefined
    WalletSetupPayload.orgId = undefined
    WalletSetupPayload.ledgerId = undefined
  ...
  async createSchema(payload: ITenantSchema): Promise<object> {
    try {
      ...
      let schemaResponse: object | PromiseLike<object>
  ...
  async createCredentialDefinition(payload: ITenantCredDef): Promise<object> {
    try {
      let credDefResponse: object | PromiseLike<object>
  ...
  async getLedgerConfigDetails(_user: IUserRequestInterface): Promise<object> {
    try {
