  ...
  async walletProvision(payload: IWalletProvision): Promise<object> {
    try {
      ...
      if (agentType === AgentType.AFJ) {
        ...
        const walletProvision = `${process.cwd() + process.env.AFJ_AGENT_SPIN_UP} ${orgId} "${externalIp}" "${walletName}" "${walletPassword}" ${seed} ${webhookEndpoint} ${walletStorageHost} ${walletStoragePort} ${walletStorageUser} ${walletStoragePassword} ${containerName} ${protocol} ${tenant} ${afjVersion} "${indyLedger}" ${inboundEndpoint} ${process.env.AWS_ACCOUNT_ID} ${process.env.S3_BUCKET_ARN} ${process.env.CLUSTER_NAME} ${process.env.FILESYSTEMID} ${process.env.ACCESSPOINTID} ${process.env.VPC_ID} ${process.env.ECS_SUBNET_ID} ${process.env.ALB_SUBNET_ID_ONE} ${process.env.ALB_SUBNET_ID_TWO} ${process.env.EFS_SECURITY_GROUP_ID} ${process.env.AWS_PUBLIC_REGION} ${process.env.STAGE} ${process.env.AGENT_WEBSOCKET_PROTOCOL} ${process.env.DB_SECURITY_GROUP_ID}`;
