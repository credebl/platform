...
export class UtilityService extends BaseService {
  ...
  async createAndStoreShorteningUrl(payload): Promise<string> {
    try {
      ...
      await this.utilityRepository.saveShorteningUrl(invitationPayload);
  ...
  async getShorteningUrl(referenceId: string): Promise<object> {
    try {
      const getShorteningUrl = await this.utilityRepository.getShorteningUrl(referenceId);
  ...
  async handleLedgerAlert(emailDto: EmailDto): Promise<void> {
    ...
    const platformConfigData = await this.utilityRepository.getPlatformConfigDetails();
