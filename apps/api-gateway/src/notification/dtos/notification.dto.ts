...
@ApiExtraModels()
export class RegisterHolderForNotificationDto {
  @ApiProperty()
  ...
  @ApiProperty()
  @Transform(({ value }) => trim(value))
  @IsNotEmpty({ message: 'holderDid is required.' })
  @IsString({ message: 'holderDid must be in string format.' })
  holderDid: string;

  @ApiProperty()
  @Transform(({ value }) => trim(value))
  @IsNotEmpty({ message: 'sessionId is required.' })
  @IsString({ message: 'sessionId must be in string format.' })
  sessionId: string;
}
