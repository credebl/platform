...
export class VerificationController {
  ...
  async sendPresentationRequest(
    ...
    for (const attrData of requestProof.attributes) {
      if (0 === temp.length) {
        temp.push(Object.values(attrData)[0]);
      } else if (!temp.includes(Object.values(attrData)[0])) {
        temp.push(Object.values(attrData)[0]);
      } else {
  ...
  async webhookProofPresentation(
    ...
    const webhookUrl = await this.verificationService
      ._getWebhookUrl(proofPresentationPayload.contextCorrelationId)
      .catch((error) => {
  ...
  async validateAttribute(attrData: object): Promise<void> {
