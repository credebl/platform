...
import { IDidList, IGetOrgById, IGetOrganization } from './interfaces/organization.interface';
import { IOrgUsers } from './interfaces/user.interface';
import {
  IOrgCredentials,
  IOrganization,
...
import { IClientRoles } from './interfaces/organization.interface';
...
export class OrganizationService {
  protected logger = new Logger('OrganizationService');
  ...
  async getOrganizations(organizationDto: GetAllOrganizationsDto, userId: string): Promise<IGetOrganization> {
    ...
    return this.natsClient.sendNatsMessage(this.serviceProxy, 'get-organizations', payload);
  }

  async getPublicOrganizations(paginationDto: PaginationDto): Promise<IGetOrganization> {
    const payload = { ...paginationDto };
    return this.natsClient.sendNatsMessage(this.serviceProxy, 'get-public-organizations', payload);
  }
