...
const migrateOrgAgentDids = async (): Promise<void> => {
  try {
    ...
    const orgDids = orgAgents.map((agent) => agent.orgDid).filter((did) => did !== null && did !== '')
    ...
    const filteredOrgAgents = orgAgents.filter((agent) => agent.orgDid !== null && agent.orgDid !== '')
...
const addSchemaType = async (): Promise<void> => {
  try {
    ...
    if (emptyTypeSchemaList.length > 0) {
