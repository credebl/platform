...
import type { ProofRequestType } from '../enum/verification.enum'
import type { IProofFormats } from '../interfaces/verification.interface'
...
export class RequestProofDtoV2 extends RequestProofDtoBase {
  ...
  @ArrayMaxSize(Number(process.env.PROOF_REQ_CONN_LIMIT), {
    message: `Limit reached (${process.env.PROOF_REQ_CONN_LIMIT} connections max).`,
  })
...
export class SendProofRequestPayload {
  ...
  @ArrayMaxSize(Number(process.env.OOB_BATCH_SIZE), {
    message: `Limit reached (${process.env.OOB_BATCH_SIZE} proof request max).`,
  })
  @IsArray()
  @IsString({ each: true, message: 'Each emailId in the array should be a string' })
  @IsOptional()
