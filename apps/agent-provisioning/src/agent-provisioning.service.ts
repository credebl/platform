  ...
  async walletProvision(payload: IWalletProvision): Promise<object> {
    try {
      ...
      if (agentType === AgentType.AFJ) {
        ...
        const spinUpResponse: Promise<object> = new Promise((resolve, reject) => {
          execFile(scriptPath, args, async (_err, stdout, stderr) => {
            ...
            const endpointFile = `${process.cwd()}${process.env.AFJ_AGENT_ENDPOINT_PATH}${orgId}_${containerName}.json`
            const tokenFile = `${process.cwd()}${process.env.AFJ_AGENT_TOKEN_PATH}${orgId}_${containerName}.json`

            const endpointExists = await this.checkFileExistence(endpointFile)
            const tokenExists = await this.checkFileExistence(tokenFile)

            if (!endpointExists || !tokenExists) {
              return reject(new NotFoundException(`${endpointFile} or ${tokenFile} files do not exist`))
            }

            const agentEndPoint = fs.readFileSync(endpointFile, 'utf8')
            const agentToken = fs.readFileSync(tokenFile, 'utf8')
