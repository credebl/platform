import type { ICredDefData, IPlatformCredDefDetails } from '@credebl/common/interfaces/cred-def.interface'
...
import type { credential_definition, org_agents, org_agents_type, organisation, schema } from '@prisma/client'
import type { ISchemaResponse } from '../interfaces'
...
import type { BulkCredDefSchema, CredDefSchema } from '../interfaces/credential-definition.interface'
...
export class CredentialDefinitionRepository {
  ...
  async getAllCredDefsByOrgIdForBulk(payload: BulkCredDefSchema): Promise<CredDefSchema[]> {
    try {
      ...
      return matchingSchemas.filter((schema) => schema !== null) as CredDefSchema[]
    } catch (error) {
