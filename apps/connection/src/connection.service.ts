...
import { ConnectionProcessState, OrgAgentType } from '@credebl/enum/enum'
import type { IUserRequest } from '@credebl/user-request/user-request.interface'
import { CACHE_MANAGER } from '@nestjs/cache-manager'
import { HttpException, HttpStatus, Inject, Injectable, type Logger, NotFoundException } from '@nestjs/common'
import { type ClientProxy, RpcException } from '@nestjs/microservices'
import { RecordType, type user } from '@prisma/client'
import type { agent_invitations } from '@prisma/client'
import type { IConnectionDetailsById } from 'apps/api-gateway/src/interfaces/IConnectionSearch.interface'
...
import type { IBasicMessage, IQuestionPayload } from './interfaces/messaging.interfaces'
@Injectable()
export class ConnectionService {
  ...
  async getConnections(
    ...
    try {
      ...
      if (getConnectionList.connectionCount === 0) {
      ...
      const connectionResponse: {
        ...
        hasPreviousPage: connectionSearchCriteria.pageNumber > 1,
  ...
  async getConnectionsById(_user: IUserRequest, connectionId: string, orgId: string): Promise<IConnectionDetailsById> {
    try {
      const agentDetails = await this.connectionRepository.getAgentEndPoint(orgId)
      const orgAgentType = await this.connectionRepository.getOrgAgentType(agentDetails?.orgAgentTypeId)
  ...
  async getAgentUrl(
    ...
    try {
      let url: string
      if (connectionInvitationFlag === 'connection-invitation') {
        if (orgAgentType === OrgAgentType.DEDICATED) {
  ...
  async createConnectionInvitation(payload: ICreateOutOfbandConnectionInvitation): Promise<ICreateConnectionUrl> {
    try {
      ...
      let legacyinvitationDid: string
  ...
  async deleteConnectionRecords(orgId: string, user: user): Promise<IDeletedConnectionsRecord> {
    try {
      ...
      if (deleteConnections?.deleteConnectionRecords?.count === 0) {
